...
Usage:
    uv run python scripts/figure_code/fig_spatial_expression_v2.py <cell_type> <gene>
    uv run python scripts/figure_code/fig_spatial_expression_v2.py Oligo_NN Sgk1
    uv run python scripts/figure_code/fig_spatial_expression_v2.py hippo Sgk1
"""
    ...
    for sample_info in samples:
        ...
        if hkey is not None and "line_split" in hparams:
            ...
            if "alignment" in hparams:
                ...
                x_bg = df_bg["x"].to_numpy(float)
                y_bg = df_bg["y"].to_numpy(float)
                x_off = (-x_bg.min() + x_anchor) if x_anchor >= 0 else (-x_bg.max() + x_anchor)
